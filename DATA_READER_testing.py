...
df=pd.read_csv("Daily_Sunspots_2yrs.txt", skiprows=65, skipfooter=3, delim_whitespace=True, 
...
plt.title("Sunspots in the time interval of STEREO-B launch.")
plt.plot(t1, sunspots, "r-", label="Sunspots")
...
"""
df=pd.read_csv("DST_index_2008.txt", skiprows=65, skipfooter=3, delim_whitespace=True, 
names=['dt','tm', 'DST'])
temp=df[['dt', 'tm']].apply(lambda x: ' '.join(x), axis=1)
df['ut']=pd.to_datetime(temp, format='%d-%m-%Y %H:%M:%S.%f')
...
df['DST'][df['DST']<=-100000]=np.nan

plt.subplot(311)
plt.title("Magnetic fields of referances for one-year period.")
...
df=pd.read_csv("STB_data_2009_B.txt", skiprows=71, skipfooter=3, delim_whitespace=True, 
names=['dt','tm', 'mag'])
...
plt.subplot(312)
plt.plot(df['ut'], df['mag'], label="STEREO-B magnetic field")
plt.xlabel("Time, Date format YYYYMM")
plt.ylabel("|B| [nT]")
plt.legend()
"""
...
#####ACE magnetic field####
df=pd.read_csv("ACE_data_2009_B.txt", skiprows=59, skipfooter=3, delim_whitespace=True, 
names=['dt','tm', 'Bmag'])
temp=df[['dt', 'tm']].apply(lambda x: ' '.join(x), axis=1)
df['ut']=pd.to_datetime(temp, format='%d-%m-%Y %H:%M:%S.%f')
...
plt.subplot(313)
plt.plot(df['ut'], df['Bmag'], label="ACE magnetic field")
plt.xlabel("Time, Date format YYYYMM")
plt.ylabel("|B| [nT]")
plt.legend()
plt.show()
...
comparison=STB_mag*ACE_B
plt.subplot(311)
plt.title("Comparison of the magnetic field data from ACE and STEREO-B. Time span of 1 year & 11 months.")
plt.plot(t4, ACE_B, label="ACE data $B_1$")
plt.xlabel("Time, Date format YYYYMM")
plt.ylabel("|B| [nT]")
plt.legend()
plt.subplot(312)
plt.plot(t4, STB_mag, label="STEREO-B data $B_2$")
plt.xlabel("Time, Date format YYYYMM")
plt.ylabel("|B| [nT]")
plt.legend()
plt.subplot(313)
plt.plot(t4, comparison, label="Product $B_1 \\cdot B_2$")
plt.xlabel("Time, Date format YYYYMM")
plt.ylabel("$|B|^2$ [$(nT)^2$]")
plt.legend()
plt.show()
