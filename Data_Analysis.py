...
def sunspots():
    """
    Data anaysis function for sunspots
    """
    #####Sunspots for jan-sep##########
    df=pd.read_csv("Daily_Sunspots_20070101_20070901.txt", \
    skiprows=66, skipfooter=3, delim_whitespace=True,
...
def DST_Index():
    """
    DST-Index data analysis function
    """
    ######DST-index for jan-sep##########
    df=pd.read_csv("DST_index_2007_jan_sep.txt", \
    skiprows=66, skipfooter=3, delim_whitespace=True,
    names=['dt','tm', 'DST'])
    temp=df[['dt', 'tm']].apply(lambda x: ' '.join(x), axis=1)
    df['ut']=pd.to_datetime(temp, format='%d-%m-%Y %H:%M:%S.%f')
...
def STB_VALS():
    """
    Function to extract and plot data of STEREO-B
    """
    ######STB MAG, p-speed, p-density, p-temp ###########
    df=pd.read_csv("STB_data_2007.txt", \
    skiprows=74, skipfooter=3, delim_whitespace=True,
    ...
    #Declare non-physical values as NAN
    """
...
def ACE_Bfunc():
    """
    Function to extract and plot the magnetic field data from the ACE satellite
    """
    #####ACE magnetic field####
    df=pd.read_csv("ACE_data_2007_B.txt", skiprows=59,\
     skipfooter=3, delim_whitespace=True, names=['dt','tm', 'Bmag'])
    temp=df[['dt', 'tm']].apply(lambda x: ' '.join(x), axis=1)
    df['ut']=pd.to_datetime(temp, format='%d-%m-%Y %H:%M:%S.%f')
...
def ACE_RESTfunc():
    """
    Function to extract and plot the rest
    of the measured data of the ACE satellite
    """
    #####ACE REST###########
    df=pd.read_csv("ACE_data_2007_rest.txt", skiprows=64, \
    skipfooter=3, delim_whitespace=True,names=['dt','tm', 'Pdens', 'Pspe'])
    temp=df[['dt', 'tm']].apply(lambda x: ' '.join(x), axis=1)
    df['ut']=pd.to_datetime(temp, format='%d-%m-%Y %H:%M:%S.%f')
...
"""
Total data collection times in seconds and days
"""
#T=20995200 #s
T=243       #d
...
N1=int(len(DST))                #No of DST data points
fs=N1/T                         #Data collection Frequency
xaxis1=np.fft.fftfreq(N1, 1/fs) #Fourier x-axis
fft_DST=np.fft.fft(DST, N1)     #Fourier Transformation

#Do the same thing for the magnetic field.
N2=int(len(STB_mag))
fs=N2/T
deltat=1/fs
...
index=np.linspace(0, N2-1, N2, dtype=int)   #List of indices
y_uten=STB_mag[STB_mag>=-10000000]          #Note the indices which are NAN
...
for offf in sorted(a, reverse=True):
    del index_uten[offf]                    #Delete all values which are NAN

index=np.asarray(index)
index_uten=np.asarray(index_uten)
STB_mag_=np.interp(index*deltat, index_uten*deltat, y_uten) #Interpolate
...
#Fourier transformation of ACE Magnetic Field.
N3=int(len(ACE_B))
fs=N3/T
deltat=1/fs
xaxis3=np.fft.fftfreq(N3, 1/fs)

#Interpolate missing data points as previously.
